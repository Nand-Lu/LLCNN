...
from Pix2PixLoss import MixedPix2PixLoss
import torch
...
parser.add_argument('--dataset_train', type=str, default='our485',
                     help="dataset directory name")

parser.add_argument('--dataset_valid', type=str, default='our485',
                     help="dataset directory name")
parser.add_argument('--dataset_test', type=str, default='eval15/low',
                     help="dataset directory name")
...
parser.add_argument('--model', default='model_epoch_50.pth', type=str, metavar='PATH',
...
def main():
    ...
    if not opt.test:
        train_set = get_training_set(opt.dataset_train, opt.crop_size,
            opt.upscale_factor )

        validation_set = get_validation_set(
            opt.dataset_valid, opt.crop_size, opt.upscale_factor)


    test_set = get_test_set(
        opt.dataset_test, opt.crop_size, opt.upscale_factor)

    if not opt.test:

        training_data_loader = DataLoader(
            dataset=train_set, num_workers=opt.threads, batch_size=opt.batch_size, shuffle=False)
    ...
    # criterion  = SSIM(data_range=255, nonnegative_ssim=True,K=(0.0001,0.001))
    criterion = MixedPix2PixLoss()
    if opt.cuda:
